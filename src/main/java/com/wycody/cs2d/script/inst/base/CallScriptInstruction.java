...
public class CallScriptInstruction extends Instruction {
    ...
    public void process(Context context) {
        if(args == null)
            preprocess(context);
    	callText = target.getName()  + "(" + target.generateParameters(CS2Utils.getParameters(target, script.getIntegerStack(), script.getObjectStack(), script.getLongStack())) + ")";

        assert target.getReturnTypes() != null;
	...
	/**
	 * @param callText the callText to set
	 */
