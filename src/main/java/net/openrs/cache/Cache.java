...
public final class Cache implements Closeable {
	...
	public ChecksumTable createChecksumTable() throws IOException {
		...
		for (int i = 0; i < size; i++) {
			...
			if (buf != null && buf.limit() > 0) {
				ReferenceTable ref = ReferenceTable.decode(Container.decode(buf).getReadOnlyData());
	...
	public ByteBuffer read(int type, int file, int member) throws IOException {
		...
		Archive archive = Archive.decode(container.getReadOnlyData(), entry.capacity());
	...
	public void write(int type, int file, int member, ByteBuffer data) throws IOException {
		...
		if (file < store.getFileCount(type) && oldArchiveSize != -1) {
			...
			archive = Archive.decode(container.getReadOnlyData(), oldArchiveSize);
		} else {
	...
	public HashMap<Integer, Integer> printNameHashes(ReferenceTable table) {
		HashMap<Integer, Integer> nameHashes = new HashMap<Integer, Integer>();
		for(int entry = 0; entry < table.capacity(); entry++) {
			try {
				if(table.getEntry(entry).getIdentifier() != -1)
					nameHashes.put(table.getEntry(entry).getIdentifier(), entry);
	...
	public int findName(ReferenceTable table, String name, int entry) {
		if(name == null) {
			return -1;
		}
		int hash = Djb2.djb2(name);
		for(int child = 0; child < table.getEntry(entry).capacity(); child++) {
			try {
				if(table.getEntry(entry, child).getIdentifier() == hash) {
					return child;
				}
			} catch(NullPointerException e) {
	...
	public int findName(ReferenceTable table, String name, int entry, int child) {
		if(name == null) {
			return -1;
		}
		int hash = Djb2.djb2(name);
