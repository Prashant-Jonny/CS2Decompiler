...
import net.openrs.util.CompressionUtils;
...
public final class Container {
	...
	public static Container decode(ByteBuffer buffer, int[] key) throws IOException {
		...
		if (type == COMPRESSION_NONE) {
			...
			if (uncompressed.length != uncompressedLength) {
				throw new IOException("Length mismatch! received: " + uncompressed.length + ", expected: " + uncompressedLength);
			}
	...
	public ByteBuffer encode() throws IOException {
		ByteBuffer data = getReadOnlyData(); // so we have a read only view, making this method thread safe
