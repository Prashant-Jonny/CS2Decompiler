...
public class ReferenceTable {
	...
	public static class Entry {
		...
		/**
		 * Some unknown hash added on build 816. It is hard to pinpoint what
		 * exactly this is because it is not used in the client.
		 */
		private int mysteryHash;
		...
		/**
		 * The version of this entry stored as the time the file was last edited
		...
		/**
		 * Gets the "mystery hash" of this entry.
		 * @return The {@link #mysteryHash}.
		 */
		public int getMysteryHash() {
			return mysteryHash;
		}
		
		/**
		 * Gets the compressed size of this entry.
		 * @return The {@link #compressedSize}.
		 */
		 ...
		 * @return The {@link #uncompressedSize}.
		 */
		...
		/**
		 * Sets the "mystery hash" of this entry.
		 * @param hash The "mystery hash."
		 */
		public void setMysteryHash(int hash) {
			this.mysteryHash = hash;
		}
	 ...
	 * {@link net.openrs.util.crypto.Djb2} hashed identifiers.
	 */
	...
	/**
	 * A flag which indicates this {@link ReferenceTable} contains
	 * sizes for its entries.
	 */
	public static final int FLAG_SIZES = 0x04;
	
	/**
	 * A flag which indicates this {@link ReferenceTable} contains
	 * some kind of hash which is currently unused by the RS client.
	 */
	public static final int FLAG_UNKOWN_HASH = 0x08;
	...
	public static ReferenceTable decode(ByteBuffer buffer) {
		...
		if ((table.flags & FLAG_UNKOWN_HASH) != 0) {
			for (int id : ids) {
				table.entries.get(id).mysteryHash = buffer.getInt();
			}
		}
		...
		if ((table.flags & FLAG_SIZES) != 0) {
			for (int id : ids) {
				table.entries.get(id).compressedSize = buffer.getInt();
				table.entries.get(id).uncompressedSize = buffer.getInt();
			}
		}
